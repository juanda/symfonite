...
class symfoniteConnector
{
    ...
    public function getAttributes()
    {
        ...
        // permission given to the user through the sftGuardPlugin
        $c = new Criteria();
        $c->add(SftUsuarioPeer::ID_SFUSER, $this -> sfGuardUser -> getId());

        $edaUser = SftUsuarioPeer::doSelectOne($c);
        if ($edaUser instanceof SftUsuario)
        {
            $attributes['id_sfuser'] = $edaUser->getSfGuardUser()->getId();
            ...
            // Other Attributes
            $atributos = $edaUser->getSftUsuAtributoValors();
            foreach ($atributos as $atributo)
            {
                $attributes[$atributo->getSftUsuAtributos()->getNombre()] =
                        $atributo ->getValor();
            }
        }
