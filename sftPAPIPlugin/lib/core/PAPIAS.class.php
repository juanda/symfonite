...
//        exit;
    ...
    {
        $fp = fopen(sfConfig::get('app_sft_papi_plugin_as_pkey_file'), 'r');
        if (!$fp)
        {
            throw new Exception('private key file is missing');
        }

        $asId = sfConfig::get('app_sft_papi_plugin_as_id', 'example_AS');
        $pKey = fread($fp, filesize(sfConfig::get('app_sft_papi_plugin_as_pkey_file')));
        $now = time();
        $ttl = sfConfig::get('app_sft_papi_plugin_as_ttl', 3600);
        $ext = $now + $ttl;
