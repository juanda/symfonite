...
<script type="text/javascript">
                ...
                <?php foreach ($ea['perfiles'] as $perfil): ?><?php //Para cada perfil ?>
                    <?php foreach ($perfil['ambitos'] as $ambito): ?><?php //Para cada uno de sus ambitos ?>
                        ...
                        <?php if($ambito['nombre'] == 'no_ambitos'): ?>
                            <td>
                            <a href="<?php echo url_for('edaGestorSesion/cambioDePerfil?eIdPerfil='.$perfil['id_perfil'].'&eIdEa='.$ea['id_ea']) ?>"><?php echo $perfil['nombre_perfil'] ?></a>
                            </td>
                            <td>
                            <input name="perfil" onclick="jQuery.ajax({ url: '<?php echo url_for('edaGestorSesion/cambioConfPersonal?eIdPerfil='.$perfil['id_perfil'].'&eIdEa='.$ea['id_ea']) ?>' });" <?php echo porDefecto(array($perfil['id_perfil'], $ambito['id'], $ea['id_ea']), $confPersonal -> getRaw('conf') )?> type="radio" />
                                <?php //echo link_to('entrar','edaGestorSesion/cambioDePerfil?eIdPerfil='.$perfil['id_perfil'].'&eIdEa='.$ea['id_ea']) ?>
                            </td>
                        <?php elseif($ambito['nombre'] == 'porasociar'): ?>
                            <td> el perfil no tiene ámbitos asociados </td>
                        <?php else: ?>
                            <td>
                                <?php echo $perfil['nombre_perfil']?>/<a href="<?php echo url_for('edaGestorSesion/cambioDePerfil?eIdPerfil='.$perfil['id_perfil'].'&eIdAmbito='.$ambito['id'].'&eIdEa='.$ea['id_ea']) ?>"><?php echo $ambito['nombre'] ?></a>
                            </td>
                            <td>
                                <input  onclick="jQuery.ajax({ url: '<?php echo url_for('edaGestorSesion/cambioConfPersonal?eIdPerfil='.$perfil['id_perfil'].'&eIdAmbito='.$ambito['id'].'&eIdEa='.$ea['id_ea']) ?>' });"  name="perfil" <?php echo porDefecto(array($perfil['id_perfil'], $ambito['id'], $ea['id_ea']), $confPersonal -> getRaw('conf') ) ?> type="radio" />
                            </td>
                            <?php //echo link_to($ambito['nombre'],'edaGestorSesion/cambioDePerfil?eIdPerfil='.$perfil['id_perfil'].'&eIdAmbito='.$ambito['id'].'&eIdEa='.$ea['id_ea']) ?>
                        <?php endif; ?>
    ...
    <?php else:?>
        <b><?php echo __('No tiene asociado ningun perfil en esta aplicaci&oacute;n');?></b>
    <?php endif; ?>
       </div>
    </div>