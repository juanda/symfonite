...
class Utilidades
{
    ...
    static public function dameUsuarioConSesionActiva($token, $id_aplicacion)
    {
        ...
        {
            $c = new Criteria();
            $c -> add(EdaAplicacionSesionesPeer::ID_APLICACION, $id_aplicacion);
            $c -> add(EdaAplicacionSesionesPeer::TOKEN, $token);

            $aplicacion_sesion = EdaAplicacionSesionesPeer::doSelect($c);
            $sesionborrada = false;
            if(count($aplicacion_sesion) == 1)
            {
                ...
                if($entiempo)
                {
                    $usuario = $aplicacion_sesion[0] -> getEdaUsuarios();
    ...
    static public function isSfBreadNavMenu()
    {
        ...
        $c -> addJoin(EdaAplicacionesPeer::SF_APP, sfBreadNavApplicationPeer::APPLICATION);
