...
class periodoActions extends autoPeriodoActions {
    ...
    public function executeDelete(sfWebRequest $request) {
        ...
        $perido = SftPeriodoPeer::retrieveByPK($request->getParameter('id'));
        if (!strcmp($request->getParameter('id'), $this->getUser()->getAttribute('idPeriodo', null, 'SftUser'))){
            $this->getUser()->setFlash('error', 'No se puede borrar el periodo asociado a la UO en la que se encuentra actualmente');
        }else if(count($perido->getSftUo()) != 0){
            $this->getUser()->setFlash('error', 'No se puede borrar un periodo con UOs asociadas a él.');
        }else{
