<?php
    ...
    if (in_array($root->getCredential(),  $escaping_on ? $sf_data->getRaw('credArray') : $credArray   )) {
    ...
    for ($i=0; $i< count($pages) ; $i++ ) {  
      ...
      $pagename = is_callable("__") ? __($pages[$i]['page']) : $pages[$i]['page']; // only translating if I18N is enabled.    	
      ...
      if ($i==$nexttop) {
         ...
         if ($havechildren){
           $open = "<li class='breadnavdivider'>". '<a class=top href=' . navbardisplayfunctions::url_for_valid(navbarfunctions::pageroute($pages[$i]['module'],$pages[$i]['action'])) . ">" . $pagename . $cssie56open."<ul>";
           $close = '</ul>' . $cssie56close ."</li>\n";
         }else{
           $open = "<li class='breadnavdivider'>". '<a class=top href=' . navbardisplayfunctions::url_for_valid(navbarfunctions::pageroute($pages[$i]['module'],$pages[$i]['action'])) . ">" . $pagename . $cssie56open;
           $close = $cssie56close ."</li>\n";
         }
      
      //no children   
      }elseif ($pages[$i]['tree_right'] - $pages[$i]['tree_left'] == 1) {                 
           $open = "<li class='standard' >"  . '<a class=standard href=' . navbardisplayfunctions::url_for_valid(navbarfunctions::pageroute($pages[$i]['module'],$pages[$i]['action'])) . ">" . $pagename . '</a>'.$cssff2;
           $close = "</li>\n";         
      //has children                 
      }else{
    
          $open = "<li class='standard'>" . '<a class=standard href=' . navbardisplayfunctions::url_for_valid(navbarfunctions::pageroute($pages[$i]['module'],$pages[$i]['action'])) . ">"  . $pagename . $cssie56open   . $cssff2 . "<ul>";
          $close =  "</ul>". $cssie56close ."</li>\n";
